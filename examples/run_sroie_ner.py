...
from my_seqeval import Seqeval
...
from layoutlmft.trainer.trainer import SROIETrainer, ROTrainingArguments

from timm.data.constants import \
    IMAGENET_DEFAULT_MEAN, IMAGENET_DEFAULT_STD, IMAGENET_INCEPTION_MEAN, IMAGENET_INCEPTION_STD
from torchvision import transforms
import torch
# set max_split_size_mb to avoid memory error
...
def get_sroie_id2label():
    id2label = {}
    with open('RO-Datasets/SROIE/labels_bio.txt') as f:
        for id, line in enumerate(f):
            id2label[id] = line.strip().upper() 
    return id2label
...
def main():
    ...
    parser = HfArgumentParser((ModelArguments, DataTrainingArguments, ROTrainingArguments))
    ...
    import layoutlmft.data.sroie_ner
    ...
    def tokenize_and_align_labels(examples, augmentation=False):
        ...
        tokenized_inputs["word_lengths"] = examples["word_lengths"]
        tokenized_inputs["word_ids"] = [tokenized_inputs.word_ids(batch_index=batch_index) 
                                            for batch_index in range(len(tokenized_inputs["input_ids"]))]
    ...
    id2label = get_sroie_id2label()
    trainer = SROIETrainer(
        model=model,
        args=training_args,
