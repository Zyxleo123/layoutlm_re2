...
Label2ids = {}
with open('./RO-Datasets/CORD/labels_bio.txt', 'r') as f:
    idx = 0
    for line in f:
        label = line.strip()
        Label2ids[label] = idx
...
Train_files = []
with open('./RO-Datasets/CORD/data.train.txt', 'r') as f:
    for line in f:
        Train_files.append(line.split('\t')[-1].split('/')[-1].strip())
    
Test_files = []
with open('./RO-Datasets/CORD/data.test.txt', 'r') as f:
    for line in f:
        Test_files.append(line.split('\t')[-1].split('/')[-1].strip())
        
Validation_files = []
with open('./RO-Datasets/CORD/data.valid.txt', 'r') as f:
    for line in f:
        Validation_files.append(line.split('\t')[-1].split('/')[-1].strip())
...
class CustomNERConfig(datasets.BuilderConfig):
    ...
    def __init__(self, data_root, anno_dir, img_dir,
                 ro_info, 
                 train_files, test_files, validation_files, **kwargs):
...
class Cord(datasets.GeneratorBasedBuilder):
    ...
    BUILDER_CONFIGS = [
        CustomNERConfig(name="default", version=datasets.Version("1.0.0"), description="Cord", 
                        data_root=os.path.join("RO-Datasets", "CORD"), anno_dir="jsons", img_dir="images",
                        ro_info=True,
                        train_files=Train_files, test_files=Test_files, validation_files=Validation_files)
    ]
    ...
    def _generate_examples(self, split):
        ...
        anno_full = os.path.join(data_root, self.config.anno_dir)
        img_full = os.path.join(data_root, self.config.img_dir)
        logger.info("⏳ Generating examples from = %s", self.config.anno_dir)
        for uid, file in enumerate(sorted(os.listdir(anno_full))):
            if split == "train" and file not in self.config.train_files or \
               split == "test" and file not in self.config.test_files or \
               split == "validation" and file not in self.config.validation_files:
                continue
            tokens = []
            bboxes = []
            ner_tags = []
            ...
            with open(file_path, "r", encoding="utf8") as f:
                data = json.load(f)
            image_path = os.path.join(img_full, data['img']['image_path'].split('/')[-1])
